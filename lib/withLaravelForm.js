...
function withLaravelForm(EnhancedComponent, options = { endPoint: null, takeAtLeast: null, params: null }) {
    ...
    return _temp = _class = class LaravelForm extends Component {
        ...
        this.submitForm = event => {
            ...
            const formData = _extends({}, this.constructFormFieldData(), this.state.formFields);

            const params = this.getOption('params');

            const args = params ? [...params, formData] : [formData];

            this.getOption('endPoint')(...args).takeAtLeast(this.getOption('takeAtLeast')).then(() => {
